...
with torch.no_grad():
    for i, row in test_df.iterrows():
        ...
        if i % 500 == 0: print(f"BERT Progress: {i}/{len(test_df)}")

# ------------------------------------------
# C. 比較と分析 (ここがレポートの宝庫)
# ------------------------------------------
print("\n=== 分析結果: BERTだけが正解した例 (SVMの敗北) ===")
count = 0
for i, (true, svm, bert, text) in enumerate(zip(test_df["label_id"], svm_preds, bert_preds, test_df["text"])):
    if bert == true and svm != true:
        ...
        if count >= 3: break # 3件見つかったら終了

print("\n" + "="*80 + "\n")

print("=== 分析結果: SVMだけが正解した例 (BERTの敗北) ===")
count = 0
for i, (true, svm, bert, text) in enumerate(zip(test_df["label_id"], svm_preds, bert_preds, test_df["text"])):
...
print("\n分析完了。この具体的なテキストをレポートの「考察」に引用しましょう！")